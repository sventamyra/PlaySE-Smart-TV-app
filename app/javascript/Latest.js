...
Latest.onLoad = function(refresh)
{
...
Latest.loadXml = function(refresh){

    requestUrl('http://www.svtplay.se/senaste?sida=1',
               function(status, data)
               {
                   data = data.responseText.split("div id=\"gridpage-content")[1];
                   data = data.split("</article>");
                   data.pop();
                   Latest.decode_data(data);
...
Latest.decode_data = function(data) {
    try {
        var html; 
        var Name;
        var Link;
        ...
        Log("Latest decode_data Exception:" + err.message + " data[" + k + "]:" + data[k]);
    }
};